...
class UserWithdrawService extends \xhadmin\CommonService
{
	...
	{
		try {
			$res = \app\accounts\model\UserWithdraw::where($where)->field($field)->order($order)->paginate(["list_rows" => $limit, "page" => $page])->toArray();
		} catch (\Exception $e) {
	...
	{
		try {
			$res = \app\accounts\model\UserWithdraw::update($data);
