...
class Synchronization extends \app\BaseController
{
	protected $wqConn;
	public function index()
	{
		if ($this->request->isAjax()) {
			...
			if ($type == 1) {
				...
				$wxappid = input("wxappid", "", "trim");
				if (empty($urladdress) || empty($urladdress) || empty($username) || empty($libraryname) || empty($librarypwd) || empty($uniacid) || empty($wxappid)) {
					return json(["code" => 0, "msg" => "参数不正确请填写参数"]);
				}
				$wqConn = $this->Configs($urladdress, $username, $libraryname, $librarypwd);
				$tabconfig = $this->tabconfig();
				$getall = $this->getall($tabconfig, $uniacid, $wqConn, $wxappid);
				if ($getall["code"] == 1) {
	...
	public function datas($table, $v, $uniacid, $wqConn, $wxappid)
	{
		$data["list"] = [];
		$wxapp_id = wxappid;
		...
		if ($v == "gc_school_user") {
			foreach ($table as $key => $vs) {
				...
				$data["list"][$key]["t_name"] = $vs["username"];
	...
	public function getall($table, $uniacid, $wqConn, $wxappid)
	{
		foreach ($table as $key => $v) {
			$this->tableData($v, $uniacid, $wqConn, $wxappid);
		}
		$array = ["code" => 1, "msg" => "同步成功"];
		return $array;
	}
	public function tableData($table, $uniacid, $wqConn, $wxappid, $page = 1)
	{
		$data = $wqConn->name($table)->where("uniacid", $uniacid)->page($page, 500)->select()->toArray();
		$tpdata = $this->datas($data, $table, $uniacid, $wqConn, $wxappid);
		$this->inserts($tpdata);
		if (!empty($data)) {
