...
class UserService extends \xhadmin\CommonService
{
	...
	{
		try {
			validate("app\\admin\\validate\\User")->scene("add")->check($data);
			$data["pwd"] = md5($data["pwd"] . config("my.password_secrect"));
			$data["role_id"] = implode(",", $data["role_id"]);
			$data["create_time"] = time();
			$res = \app\admin\model\User::create($data);
	...
	{
		try {
			validate("app\\admin\\validate\\User")->scene("update")->check($data);
			$data["role_id"] = implode(",", $data["role_id"]);
	...
	{
		try {
			validate("app\\admin\\validate\\User")->scene("updatePassword")->check($data);
			$data["pwd"] = md5($data["pwd"] . config("my.password_secrect"));
			$res = \app\admin\model\User::update($data);
		} catch (\think\exception\ValidateException $e) {
			throw new \think\exception\ValidateException($e->getError());
