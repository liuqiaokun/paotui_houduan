...
class ContentService extends \xhadmin\CommonService
{
	...
	public static function add($data)
	{
		...
		try {
			...
			$res = \app\admin\model\Cms\Content::create($data);
			if ($res) {
				\app\admin\model\Cms\Content::update(["content_id" => $res->content_id, "sortid" => $res->content_id]);
	...
	public static function update($data)
	{
		...
		try {
			...
			$res = \app\admin\model\Cms\Content::update($data);
			self::saveExtData($data);
		} catch (\Exception $e) {
			abort(config("my.error_log_code"), $e->getMessage());
		}
	...
	{
		$fieldsetInfo = \app\admin\model\Cms\Catagory::find($data["class_id"]);
		try {
	...
	public static function delete($where)
	{
		try {
			foreach ($where["content_id"] as $k => $v) {
				$contentInfo = \app\admin\model\Cms\Content::find($v);
				$classInfo = \app\admin\model\Cms\Catagory::find($contentInfo["class_id"]);
				if ($classInfo) {
			...
			$res = \app\admin\model\Cms\Content::destroy($where);
		} catch (\Exception $e) {
			abort(config("my.error_log_code"), $e->getMessage());
		}
		return $res;
	...
	{
		$list = \app\admin\model\Cms\Catagory::where(["pid" => $pid])->field("class_id,pid,class_name,class_id as childs")->order("sortid asc,class_id asc")->select();
		foreach ($list as $key => $val) {
			$sublist = \app\admin\model\Cms\Catagory::where(["pid" => $val["class_id"]])->field("class_id,pid,class_name,class_id as childs")->order("sortid asc,class_id asc")->select();
			if ($sublist) {
				$childs = \app\admin\controller\Cms\facade\Cat::getSubClassId($list, $val["class_id"]);
	...
	{
		try {
			$contentInfo = \app\admin\model\Cms\Content::find($content_id);
			if ($contentInfo) {
