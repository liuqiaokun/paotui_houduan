...
class Catagory extends \app\admin\controller\Admin
{
	public function index()
	{
		if (!$this->request->isAjax()) {
			...
			$orderby = "sortid asc";
			$res = \app\admin\service\Cms\CatagoryService::indexList(formatWhere($where), $field, $orderby, $limit, $page);
	...
	public function updateExt()
	{
		...
		\app\admin\model\Cms\Catagory::update($data);
	...
	public function add()
	{
		if (!$this->request->isPost()) {
			...
			$info = \app\admin\model\Cms\Catagory::find($class_id);
			$data["type"] = $info->type;
			...
			$this->view->assign("tpList", \app\admin\service\Cms\CatagoryService::tplList($default_themes));
			return view("cms/catagory/add");
		} else {
			$data = $this->request->post();
			$res = \app\admin\service\Cms\CatagoryService::add($data);
	...
	public function update()
	{
		if (!$this->request->isPost()) {
			...
			$default_themes = config("xhadmin.default_themes") ? config("xhadmin.default_themes") : "index";
			$this->view->assign("tpList", \app\admin\service\Cms\CatagoryService::tplList($default_themes));
			$this->view->assign("info", checkData(\app\admin\model\Cms\Catagory::find($class_id)));
			...
			\app\admin\service\Cms\CatagoryService::update($data);
	...
	public function delete()
	{
		...
		\app\admin\model\Cms\Catagory::destroy(["class_id" => explode(",", $idx)]);
		return json(["status" => "00", "msg" => "操作成功"]);
	}
	public function setSort()
	{
		...
		\app\admin\service\Cms\CatagoryService::setSort($class_id, $type);
		return json(["status" => "00", "msg" => "操作成功"]);
	}
}