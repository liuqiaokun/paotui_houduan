...
class CatagoryService extends \xhadmin\CommonService
{
	...
	public static function add($data)
	{
		...
		try {
			$res = \app\admin\model\Cms\Catagory::create($data);
			if ($res->class_id) {
				\app\admin\model\Cms\Catagory::update(["class_id" => $res->class_id, "sortid" => $res->class_id]);
			}
		} catch (\Exception $e) {
			abort(config("my.error_log_code"), $e->getMessage());
		}
	...
	public static function update($data)
	{
		...
		try {
			$res = \app\admin\model\Cms\Catagory::update($data);
		} catch (\Exception $e) {
			abort(config("my.error_log_code"), $e->getMessage());
		}
		return $res;
	...
	{
		$data = \app\admin\model\Cms\Catagory::find($class_id);
		if ($type == 1) {
			$where["sortid"] = ["<", $data["sortid"]];
			$where["pid"] = $data["pid"];
			$info = \app\admin\model\Cms\Catagory::where(formatWhere($where))->order("sortid desc")->find();
		} else {
			$where["sortid"] = [">", $data["sortid"]];
			$where["pid"] = $data["pid"];
			$info = \app\admin\model\Cms\Catagory::where(formatWhere($where))->order("sortid asc")->find();
		}
		if ($info && $data) {
			\app\admin\model\Cms\Catagory::update(["class_id" => $class_id, "sortid" => $info->sortid]);
			\app\admin\model\Cms\Catagory::update(["class_id" => $info->class_id, "sortid" => $data->sortid]);
		}
	}
	...
	public static function getFilepath($classname, $classId)
	{
		...
		$info = \app\admin\model\Cms\Catagory::where(formatWhere($where))->find();
		if (empty($info)) {
			return $filepath;
		} else {
