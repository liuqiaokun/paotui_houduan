...
class ActionService extends \xhadmin\CommonService
{
	public static function saveData($type, $data)
	{
		try {
			...
			if ($type == "add") {
				$info = \app\admin\controller\Sys\model\Action::where(["menu_id" => $data["menu_id"], "type" => $data["type"], "action_name" => $data["action_name"]])->find();
				if (!$info) {
					$reset = \app\admin\controller\Sys\model\Action::create($data);
					if ($reset->id) {
						\app\admin\controller\Sys\model\Action::update(["id" => $reset->id, "sortid" => $reset->id]);
						if ($data["type"] == 32) {
							$hszActionList = [];
				...
				$reset = \app\admin\controller\Sys\model\Action::update($data);
	...
	public static function addFast($data)
	{
		...
		$postField = \app\admin\controller\Sys\model\Field::where(["is_post" => 1, "menu_id" => $data["menu_id"]])->column("field");
		if ($actions) {
			try {
				foreach ($actions as $key => $val) {
					...
					$info = \app\admin\controller\Sys\model\Action::where(["menu_id" => $data["menu_id"], "action_name" => $actionInfo[1]])->find();
					if (!$info) {
	...
	{
		$data = \app\admin\controller\Sys\model\Action::find($id);
		if ($type == 1) {
			$map = "sortid < " . $data["sortid"] . " and menu_id = " . $data["menu_id"];
			$info = \app\admin\controller\Sys\model\Action::where($map)->order("sortid desc")->find();
		} else {
			$map = "sortid > " . $data["sortid"] . " and menu_id = " . $data["menu_id"];
			$info = \app\admin\controller\Sys\model\Action::where($map)->order("sortid asc")->find();
		}
		try {
			if ($info && $data) {
				\app\admin\controller\Sys\model\Action::update(["id" => $id, "sortid" => $info["sortid"]]);
				\app\admin\controller\Sys\model\Action::update(["id" => $info["id"], "sortid" => $data["sortid"]]);
			} else {
