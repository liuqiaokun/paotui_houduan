...
class BuildService extends \xhadmin\CommonService
{
	...
	public static function getRelateFieldList($table_name)
	{
		...
		try {
			...
			$fieldList = \app\admin\controller\Sys\model\Field::where($map)->select()->toArray();
		} catch (\Exception $e) {
			return false;
		}
		return $fieldList;
	...
	{
		$info = \app\admin\controller\Sys\model\Field::where(["field" => $fieldName, "menu_id" => $menu_id])->find();
		if ($info) {
			return $info->type;
		}
	}
