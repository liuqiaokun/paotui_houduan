...
class User extends Admin
{
	...
	public function add()
	{
		if (!$this->request->isPost()) {
			...
			$res = \app\admin\service\UserService::add($data);
	...
	public function update()
	{
		if (!$this->request->isPost()) {
			...
			$this->view->assign("info", checkData(\app\admin\model\User::find($user_id)));
			return view("update");
		} else {
			$postField = "user_id,name,user,role_id,note,status,create_time";
			$data = $this->request->only(explode(",", $postField), "post", null);
			$res = \app\admin\service\UserService::update($data);
	...
	public function updatePassword()
	{
		if (!$this->request->isPost()) {
			...
			\app\admin\service\UserService::updatePassword($data);
	...
	public function delete()
	{
		...
		\app\admin\model\User::destroy(["user_id" => explode(",", $idx)]);
		return json(["status" => "00", "msg" => "操作成功"]);
	}
}