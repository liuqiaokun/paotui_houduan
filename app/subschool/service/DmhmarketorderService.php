...
class DmhmarketorderService extends \xhadmin\CommonService
{
	...
	{
		try {
			$res = \app\subschool\model\Dmhmarketorder::where($where)->field($field)->order($order)->paginate(["list_rows" => $limit, "page" => $page])->toArray();
		} catch (\Exception $e) {
	...
	{
		try {
			$res = \app\subschool\model\Dmhmarketorder::update($data);
