...
class StoreRechargeService extends \xhadmin\CommonService
{
	...
	{
		try {
			$res = \app\subschool\model\StoreRecharge::where($where)->field($field)->order($order)->paginate(["list_rows" => $limit, "page" => $page])->toArray();
		} catch (\Exception $e) {
	...
	public static function add($data)
	{
		try {
			...
			$data["addtime"] = time();
			$res = \app\subschool\model\StoreRecharge::create($data);
	...
	public static function update($data)
	{
		try {
			$data["wxapp_id"] = session("subschool.wxapp_id");
			$data["s_id"] = session("subschool.s_id");
			$res = \app\subschool\model\StoreRecharge::update($data);
		} catch (\think\exception\ValidateException $e) {
			throw new \think\exception\ValidateException($e->getError());
		} catch (\Exception $e) {
			abort(config("my.error_log_code"), $e->getMessage());
