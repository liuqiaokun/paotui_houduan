...
class DmharticleService extends \xhadmin\CommonService
{
	...
	{
		try {
			$res = \app\subschool\model\Dmharticle::where($where)->field($field)->order($order)->paginate(["list_rows" => $limit, "page" => $page])->toArray();
		} catch (\Exception $e) {
	...
	public static function add($data)
	{
		try {
			...
			$res = \app\subschool\model\Dmharticle::create($data);
	...
	public static function update($data)
	{
		try {
			...
			$data["wxapp_id"] = session("subschool.wxapp_id");
			$data["s_id"] = session("subschool.s_id");
			$res = \app\subschool\model\Dmharticle::update($data);
		} catch (\think\exception\ValidateException $e) {
			throw new \think\exception\ValidateException($e->getError());
		} catch (\Exception $e) {
			abort(config("my.error_log_code"), $e->getMessage());
