...
class Upload extends Admin
{
	private function upload($filekey)
	{
		...
		$school = \think\facade\Db::name("school")->where("s_id", session("subschool.s_id"))->find();
		$oss = \think\facade\Db::name("upload_oss")->where("wxapp_id", $school["wxapp_id"])->find();
		...
		if ($upload_hash_status && $fileinfo && $this->checkFileExists($fileinfo["filepath"])) {
			...
			$url = $this->up($file, $upload_config_id, $oss, $suffix);
		}
		return $url;
	}
	protected function up($file, $upload_config_id, $oss, $suffix)
	{
		try {
			if ($oss["oss_status"]) {
				...
				$url = \utils\oss\OssService::getAdminFileName(basename($info), $suffix);
