...
class Dmharticle extends Admin
{
		...
		if (in_array($this->request->action(), ["updateExt", "update", "delete", "view"])) {
			...
			if ($idx) {
				foreach (explode(",", $idx) as $v) {
					$info = \app\subschool\model\Dmharticle::find($v);
	...
	public function index()
	{
		if (!$this->request->isAjax()) {
			...
			$res = \app\subschool\service\DmharticleService::indexList(formatWhere($where), $field, $orderby, $limit, $page);
			foreach ($res["rows"] as $k => $v) {
				$rows[$k] = $v;
				$rows[$k]["pages"] = "/gc_school/pages/dmharticle/article?id=" . $v["id"];
			}
			$res["rows"] = $rows;
			return json($res);
		}
	...
	public function add()
	{
		if (!$this->request->isPost()) {
			...
			$res = \app\subschool\service\DmharticleService::add($data);
	...
	public function update()
	{
		if (!$this->request->isPost()) {
			...
			$this->view->assign("info", checkData(\app\subschool\model\Dmharticle::find($id)));
			return view("update");
		} else {
			$postField = "id,,title,content,createtime,wxapp_id,s_id";
			$data = $this->request->only(explode(",", $postField), "post", null);
			$res = \app\subschool\service\DmharticleService::update($data);
	...
	public function delete()
	{
		...
		try {
			\app\subschool\model\Dmharticle::destroy(["id" => explode(",", $idx)], true);
		} catch (\Exception $e) {
	...
	public function view()
	{
		...
		$this->view->assign("info", \app\subschool\model\Dmharticle::find($id));
		return view("view");
	}
}