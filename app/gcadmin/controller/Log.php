...
class Log extends Admin
{
	public function index()
	{
		if (!$this->request->isAjax()) {
			...
			$res = \app\gcadmin\service\LogService::indexList(formatWhere($where), $field, $orderby, $limit, $page);
			return json($res);
		}
	}
	public function delete()
	{
		...
		try {
			\app\gcadmin\model\Log::destroy(["id" => explode(",", $idx)], true);
		} catch (\Exception $e) {
	...
	public function view()
	{
		...
		$this->view->assign("info", \app\gcadmin\model\Log::find($id));
		return view("view");
	}
	public function dumpData()
	{
		...
		try {
			...
			$list = \app\gcadmin\model\Log::where(formatWhere($where))->order("id desc")->select();
			if (empty($list)) {
				throw new Exception("没有数据");
			}
			\app\gcadmin\service\LogService::dumpData(htmlOutList($list), filterEmptyArray(array_unique($fieldInfo)));
		} catch (\Exception $e) {
