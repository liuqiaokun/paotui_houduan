...
class Login extends Admin
{
	public function index()
	{
		if (!$this->request->isPost()) {
			...
			if ($this->checkLogin($data)) {
				$this->success("登录成功", url("gcadmin/Index/index"));
			}
		}
	}
	private function checkLogin($data)
	{
		...
		session("gcadmin", $info);
		session("gcadmin_sign", data_auth_sign($info));
		event("LoginLog", $info);
		return true;
	}
	...
	public function out()
	{
		session("gcadmin", null);
		session("gcadmin_sign", null);
		return redirect(url("gcadmin/Login/index"));
	}
