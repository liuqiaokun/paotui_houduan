...
class UploadConfig extends Admin
{
	public function index()
	{
		if (!$this->request->isAjax()) {
			...
			$res = \app\gcadmin\service\UploadConfigService::indexList(formatWhere($where), $field, $orderby, $limit, $page);
			return json($res);
		}
	}
	public function updateExt()
	{
		...
		try {
			\app\gcadmin\model\UploadConfig::update($data);
		} catch (\Exception $e) {
	...
	public function add()
	{
		if (!$this->request->isPost()) {
			...
			$data = $this->request->only(explode(",", $postField), "post", null);
			$res = \app\gcadmin\service\UploadConfigService::add($data);
	...
	public function update()
	{
		if (!$this->request->isPost()) {
			...
			$this->view->assign("info", checkData(\app\gcadmin\model\UploadConfig::find($id)));
			return view("update");
		} else {
			$postField = "id,title,upload_replace,thumb_status,thumb_width,thumb_height,thumb_type";
			$data = $this->request->only(explode(",", $postField), "post", null);
			$res = \app\gcadmin\service\UploadConfigService::update($data);
	...
	public function delete()
	{
		...
		try {
			\app\gcadmin\model\UploadConfig::destroy(["id" => explode(",", $idx)], true);
		} catch (\Exception $e) {
			abort(config("my.error_log_code"), $e->getMessage());
		}
		return json(["status" => "00", "msg" => "操作成功"]);
