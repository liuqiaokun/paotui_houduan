...
class FieldService extends \xhadmin\CommonService
{
	public static function saveData($type, $data)
	{
		try {
			...
			if ($type == "add") {
				$info = \app\gcadmin\controller\Sys\model\Field::where(["menu_id" => $data["menu_id"], "field" => $data["field"]])->find();
				$reset = \app\gcadmin\controller\Sys\model\Field::create($data);
				if ($reset->id) {
					\app\gcadmin\controller\Sys\model\Field::update(["id" => $reset->id, "sortid" => $reset->id]);
				}
				if ($data["type"] == 22) {
				...
				$res = \app\gcadmin\controller\Sys\model\Field::update($data);
				if ($res) {
					$fieldInfo = \app\gcadmin\controller\Sys\model\Field::find($data["id"]);
					if ($data["name"] == "编号" && $data["field"] != $fieldInfo["field"]) {
						\app\gcadmin\controller\Sys\model\Menu::update(["pk_id" => $data["field"], "menu_id" => $fieldInfo["menu_id"]]);
					}
				}
			}
		} catch (\Exception $e) {
	...
	{
		$data = \app\gcadmin\controller\Sys\model\Field::find($id);
		if ($type == 1) {
			$map = "sortid < " . $data["sortid"] . " and menu_id = " . $data["menu_id"];
			$info = \app\gcadmin\controller\Sys\model\Field::where($map)->order("sortid desc")->find();
		} else {
			$map = "sortid > " . $data["sortid"] . " and menu_id = " . $data["menu_id"];
			$info = \app\gcadmin\controller\Sys\model\Field::where($map)->order("sortid asc")->find();
		}
		try {
			if ($info && $data) {
				\app\gcadmin\controller\Sys\model\Field::update(["id" => $id, "sortid" => $info["sortid"]]);
				\app\gcadmin\controller\Sys\model\Field::update(["id" => $info["id"], "sortid" => $data["sortid"]]);
			} else {
