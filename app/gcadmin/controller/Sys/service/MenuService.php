...
class MenuService extends \xhadmin\CommonService
{
	public static function saveData($type, $data)
	{
		try {
			...
			if ($type == "add") {
				$controllerInfo = \app\gcadmin\controller\Sys\model\Menu::where(["app_id" => $data["app_id"], "controller_name" => $data["controller_name"]])->find();
				if ($data["controller_name"] && $controllerInfo) {
					throw new \Exception("控制器已存在");
				}
				$tableInfo = \app\gcadmin\controller\Sys\model\Menu::where(["table_status" => 1, "table_name" => $data["table_name"]])->find();
				if ($data["table_status"] && $data["table_name"] && $tableInfo) {
					throw new \Exception("数据表已存在");
				}
				$reset = \app\gcadmin\controller\Sys\model\Menu::create($data);
				if ($reset->menu_id) {
					\app\gcadmin\controller\Sys\model\Menu::update(["menu_id" => $reset->menu_id, "sortid" => $reset->menu_id]);
				...
				$reset = \app\gcadmin\controller\Sys\model\Menu::update($data);
			}
		} catch (\Exception $e) {
	...
	{
		$data = \app\gcadmin\controller\Sys\model\Menu::find($id);
		if ($type == 1) {
			$map = "sortid < " . $data["sortid"] . " and pid = " . $data["pid"];
			$info = \app\gcadmin\controller\Sys\model\Menu::where($map)->order("sortid desc")->find();
		} else {
			$map = "sortid > " . $data["sortid"] . " and pid = " . $data["pid"];
			$info = \app\gcadmin\controller\Sys\model\Menu::where($map)->order("sortid asc")->find();
		}
		try {
			if ($info && $data) {
				\app\gcadmin\controller\Sys\model\Menu::update(["menu_id" => $id, "sortid" => $info["sortid"]]);
				\app\gcadmin\controller\Sys\model\Menu::update(["menu_id" => $info["menu_id"], "sortid" => $data["sortid"]]);
			} else {
	...
	{
		try {
			$reset = \app\gcadmin\controller\Sys\model\Menu::destroy(["menu_id" => $id]);
		} catch (\Exception $e) {
			throw new \Exception($e->getMessage());
		}
		return $reset;
