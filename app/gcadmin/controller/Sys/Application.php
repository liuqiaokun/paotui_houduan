...
class Application extends \app\gcadmin\controller\Admin
{
	...
	public function index()
	{
		if (!$this->request->isAjax()) {
			...
			try {
				$data["rows"] = model\Application::order("app_id desc")->select();
				$data["total"] = model\Application::count();
			} catch (\Exception $e) {
				exit($e->getMessage());
			}
			return json($data);
	...
	public function add()
	{
		if (!$this->request->isPost()) {
			...
			try {
				service\ApplicationService::saveData("add", $data);
			} catch (\Exception $e) {
	...
	public function update()
	{
		if (!$this->request->isPost()) {
			...
			$info = model\Application::find($app_id);
			if (!$info) {
				$this->error("没有数据");
			}
			...
			$data = $this->request->post();
			try {
				service\ApplicationService::saveData("edit", $data);
			} catch (\Exception $e) {
				$this->error($e->getMessage());
			}
	...
	public function delete()
	{
		...
		try {
			model\Application::destroy($app_id);
		} catch (\Exception $e) {
			$this->error($e->getMessage());
		}
	...
	public function createApplication()
	{
		...
		try {
			...
			switch ($applicationInfo["app_type"]) {
				case 1:
					$this->read_admin_all($rootPath . "app/gcadmin/controller/Sys/template/admin", $applicationInfo);
					break;
				case 2:
					$this->read_api_all($rootPath . "app/gcadmin/controller/Sys/template/api", $applicationInfo);
					break;
				case 3:
					$this->read_cms_all($rootPath . "app/gcadmin/controller/Sys/template/cms", $applicationInfo);
					...
					if (!in_array(config("database.connections.mysql.prefix") . "catagory", $array)) {
						$file = $rootPath . "app/gcadmin/controller/Sys/template/cms.sql";
						$gz = fopen($file, "r");
						for ($i = 0; $i < 1000; $i++) {
							$sql .= str_replace("cd_", config("database.connections.mysql.prefix"), fgets($gz));
							if (preg_match("/.*;\$/", trim($sql))) {
								if (false !== \think\facade\Db::query($sql)) {
									$start += strlen($sql);
								} else {
									return false;
								}
	...
	public function read_admin_all($dir, $applicationInfo)
	{
		if (!is_dir($dir)) {
			return false;
		}
			...
			while (($fl = readdir($handle)) !== false) {
				...
				if (is_dir($temp) && $fl != "." && $fl != "..") {
					...
					if ($fl != "." && $fl != "..") {
						...
						$filepath = str_replace($rootPath . "app/gcadmin/controller/Sys/template/admin", $rootPath . "app/" . $applicationInfo["app_dir"], $file);
	...
	public function read_api_all($dir, $applicationInfo)
	{
		if (!is_dir($dir)) {
			return false;
		}
			...
			while (($fl = readdir($handle)) !== false) {
				...
				if (is_dir($temp) && $fl != "." && $fl != "..") {
					...
					if ($fl != "." && $fl != "..") {
						...
						$file = str_replace("\\", "/", $temp);
						$content = str_replace("ApplicationName", $applicationInfo["app_dir"], file_get_contents($file));
						$filepath = str_replace($rootPath . "app/gcadmin/controller/Sys/template/api", $rootPath . "app/" . $applicationInfo["app_dir"], $file);
	...
	public function read_cms_all($dir, $applicationInfo)
	{
		if (!is_dir($dir)) {
			return false;
		}
		...
		if ($handle) {
			while (($fl = readdir($handle)) !== false) {
				...
				if (is_dir($temp) && $fl != "." && $fl != "..") {
					...
					$filepath = str_replace($rootPath . "app/gcadmin/controller/Sys/template/cms", $rootPath, $file);
