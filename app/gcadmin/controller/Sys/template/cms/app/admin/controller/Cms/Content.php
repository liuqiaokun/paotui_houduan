...
class Content extends \app\gcadmin\controller\Admin
{
	public function index()
	{
		if (!$this->request->isAjax()) {
			$list = json_encode(\app\gcadmin\service\Cms\ContentService::getSubClass("0"));
			...
			$res = \app\gcadmin\service\Cms\ContentService::indexList(formatWhere($where), $field, $orderby, $limit, $page);
			$list = $res["rows"];
			foreach ($list as $key => $val) {
				...
				if (!empty($val["position"])) {
					$list[$key]["title"] .= "&nbsp;" . \app\gcadmin\service\Cms\ContentService::getPositionName($val["position"], $val["content_id"]);
				}
	...
	public function add()
	{
		if (!$this->request->isPost()) {
			...
			$res = \app\gcadmin\service\Cms\ContentService::add($data);
	...
	public function update()
	{
		if (!$this->request->isPost()) {
			...
			$this->view->assign("info", checkData(\app\gcadmin\model\Cms\Content::find($content_id)->toArray()));
			return view("cms/content/info");
		} else {
			$data = $this->request->post();
			\app\gcadmin\service\Cms\ContentService::update($data);
	...
	public function delete()
	{
		...
		\app\gcadmin\service\Cms\ContentService::delete(["content_id" => explode(",", $idx)]);
		return json(["status" => "00", "msg" => "操作成功"]);
	}
	public function delPosition()
	{
		...
		\app\gcadmin\service\Cms\ContentService::delPosition($content_id, $position_id);
		return json(["status" => "00", "msg" => "操作成功"]);
	}
	public function move()
	{
		...
		$res = \app\gcadmin\model\Cms\Content::where(["content_id" => explode(",", $content_ids)])->update($data);
		return json(["status" => "00", "msg" => "操作成功"]);
	}
	public function setPosition()
	{
		...
		if ($idx) {
			foreach ($idx as $id) {
				\app\gcadmin\service\Cms\ContentService::setPosition($id, $position_id);
			}
		}
		return json(["status" => "00", "msg" => "操作成功"]);
	}
	public function getExtends()
	{
		$class_id = $this->request->post("class_id", "", "intval");
		$content_id = $this->request->post("content_id", "", "intval");
		$classInfo = \app\gcadmin\model\Cms\Catagory::find($class_id);
		...
		if ($fieldList) {
			foreach ($fieldList as $key => $val) {
				...
				$htmlstr .= \app\gcadmin\service\Cms\ContentService::getFieldData($val);
			}
