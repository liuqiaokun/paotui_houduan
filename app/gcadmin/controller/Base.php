...
class Base extends Admin
{
	...
	public function auth()
	{
		if (!$this->request->isAjax()) {
			...
			$info = \app\gcadmin\model\Role::find($role_id);
			$parentInfo = \app\gcadmin\model\Role::find($info->pid);
			$parentAccess = db("access")->where("role_id", $parentInfo->role_id)->column("purviewval", "id");
			$access = db("access")->where("role_id", $role_id)->column("purviewval", "id");
			$nodes = $this->getNodes(0, $access, $parentAccess, $info);
			$cmsMenu = (include app()->getRootPath() . "/app/gcadmin/controller/Cms/access.php");
