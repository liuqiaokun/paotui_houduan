...
class Index extends Admin
{
	public function index()
	{
		...
		$cmsMenu = (include app()->getRootPath() . "/app/gcadmin/controller/Cms/config.php");
		if ($cmsMenu) {
	...
	private function getSubMenu($pid)
	{
		...
		if ($list) {
			foreach ($list as $key => $val) {
				...
				$menus[$key]["url"] = !empty($val["url"]) ? strpos($val["url"], "://") ? $val["url"] : url($val["url"]) : url("gcadmin/" . str_replace("/", ".", $val["controller_name"]) . "/index");
				$menus[$key]["access_url"] = !empty($val["url"]) ? $val["url"] : "gcadmin/" . str_replace("/", ".", $val["controller_name"]);
			}
			return $menus;
		}
	}
