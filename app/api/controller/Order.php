...
class Order extends Common
{
	...
	public function helpBuyOrder()
	{
		...
		$data["sh_phone"] = $addres["phone"];
		$data["sh_addres"] = $addres["addres"];
		$data["total"] = $store["delivery_fee"] + $store["deliveryfee"];
		$data["createtime"] = time();
		if ($data["givetype"] == 1) {
	...
	{
		$postField = "a_id,givetype,sh_name,ordertime,phone,unique,s_id,wxapp_id,total,img,remarks,sex_limit,weight,express_num,sh_addres,qu_longitude,qu_latitude,co_id,out_time_num,start_time,good_details,food_money,store_id,MinRunPrice,goods_list,arrival_time,is_self_lifting";
		$data = $this->request->only(explode(",", $postField), "post", null);
		if ($data["unique"]) {
			$cache = \think\facade\Cache::get($data["unique"]);
			if ($cache) {
		...
		if (!$data["arrival_time"]) {
			return $this->ajaxReturn($this->errorCode, "请选择到店时间");
		}
		$store = \think\facade\Db::name("zh_business")->find($data["store_id"]);
		$school = \think\facade\Db::name("school")->find($data["s_id"]);
		$data["is_open_pay"] = 0;
	...
	public function orderLst()
	{
		...
		foreach ($res["list"] as &$v) {
			...
			$business = \think\facade\Db::name("zh_business")->where("business_id", $v["store_id"])->find();
			if ($v["is_self_lifting"] != 1) {
				$v["total"] = bcsub($v["total"], $business["deliveryfee"], 2);
			}
		}
	...
	public function payResult()
	{
		...
		file_put_contents("abc.txt", $pay_type . " 时间 " . date("Y-m-d H:i:s") . PHP_EOL, FILE_APPEND);
		...
		if ($pay_type == 1) {
			...
			if ($getData["tradeStatus"] = "SUCCESS") {
				...
				if ($order["status"] == 1) {
					$status = $order["is_self_lifting"] == 1 ? 10 : 2;
					$data = ["status" => $status, "pay_time" => time()];
					...
					if ($order["is_self_lifting"] == 1) {
						...
						$data["store_money"] = $store_rate;
						$data["fx_store_money"] = $fx_store_rate;
					}
					...
					if ($order["is_self_lifting"] == 0) {
						...
						$this->selfStorePrint($order["id"], $order["wxapp_id"]);
					}
				}
			}
		} else {
	...
	public function userCancelOrder()
	{
		...
		if (!in_array($order["status"], [1, 2, 3, 10])) {
		...
		if ($order["status"] == 10) {
			...
			if (strtotime("+30 minute") > strtotime(date("Y-m-d " . $order["arrival_time"]))) {
				return $this->ajaxReturn($this->errorCode, "到店前30分钟内不可取消");
			}
			try {
	...
	public function orderInfo()
	{
		...
		if ($order["store_id"]) {
			$order["store"] = \think\facade\Db::name("zh_business")->field("business_id,business_name,business_address,deliveryfee")->find($order["store_id"]);
			$order["totals"] = bcsub($order["total"], $order["store"]["deliveryfee"], 2);
		}
	...
	public function selfStorePrint($o_id, $wxapp_id)
	{
		$order = \app\api\model\Order::where(["id" => $o_id, "wxapp_id" => $wxapp_id])->find();
		$store = \think\facade\Db::name("zh_business")->find($order["store_id"]);
		$set = \think\facade\Db::name("setting")->where("wxapp_id", $wxapp_id)->find();
	...
	public function StorePrint($o_id, $wxapp_id)
	{
		...
		if ($order["is_open_pay"] == 1) {
			...
			$store_rate1 = $store["store_money"] != null ? $store["store_money"] : $school["store_rate"];
			if (!empty($store["fx_store_money"])) {
				$fx_store_rate1 = $store["fx_store_money"];
			} else {
				$fx_store_rate1 = $school["fx_store_rate"];
			}
			$store_rate = \floatval($order["food_money"]) * \floatval($store_rate1) / 100;
			$fx_store_rate = \floatval($order["food_money"]) * \floatval($fx_store_rate1) / 100;
	...
	public function CancelGrab()
	{
		...
		if ($type == 1) {
			...
			if ($set["mp_template_cancel"]) {
				MpMsgSubscribe::cancelOrder($o_id, $wxapp_id, $reason, $openid, 1);
			}
			if ($set["template_cancel"]) {
				MsgSubscribe::cancelOrder($o_id, $wxapp_id, $reason, $openid, 1);
			}
			if ($order["type"] == 3) {
				$store = \think\facade\Db::name("zh_business")->find($order["store_id"]);
				if ($store["wxadmin_name"]) {
					$openid = \think\facade\Db::name("wechat_user")->where("u_id", $store["wxadmin_name"])->value("openid");
					if ($set["mp_template_cancel"]) {
						MpMsgSubscribe::cancelOrder($o_id, $wxapp_id, $reason, $openid, 1);
					}
					if ($set["template_cancel"]) {
						MsgSubscribe::cancelOrder($o_id, $wxapp_id, $reason, $openid, 2);
					}
				}
				if ($store["printtype"] == 2) {
			...
			if ($set["template_refuse"]) {
				MsgSubscribe::refuseOrder($o_id, $wxapp_id, $order["start_openid"]);
				MsgSubscribe::refuseOrder($o_id, $wxapp_id, $order["end_openid"]);
			}
			if ($set["template_refuse_mp"]) {
				MpMsgSubscribe::refuseOrder($o_id, $wxapp_id, $order["start_openid"]);
				MpMsgSubscribe::refuseOrder($o_id, $wxapp_id, $order["end_openid"]);
			}
		}
		return $this->ajaxReturn($this->successCode, "操作成功");
	}
	public function agreeCancel()
	{
		...
		if ($order["type"] == 3) {
			...
			if ($config["template_cancel_rider"]) {
				MsgSubscribe::cancelOrderRider($o_id, $wxapp_id, $order["end_openid"]);
			}
			if ($config["mp_template_cancel"]) {
				MpMsgSubscribe::cancelOrder($o_id, $wxapp_id, $order["cancel_reason"], $order["end_openid"], 2);
			}
		}
		\think\facade\Db::startTrans();
	...
	public function uploadProof()
	{
		...
		if ($res = \app\api\model\Order::where(["id" => $o_id])->update(["proof" => $proof])) {
			\think\facade\Db::name("dmh_school_order")->where(["id" => $o_id, "wxapp_id" => $wxapp_id])->update(["status" => 11]);
			return $this->ajaxReturn($this->successCode, "操作成功");
		} else {
			return $this->ajaxReturn($this->errorCode, "操作失败");
		}
	...
	public function confirmFinish()
	{
		...
		try {
			...
			$res1 = \app\api\model\Order::where(["id" => $o_id])->update(["status" => "4"]);
			if ($order["is_open_pay"] == 1 && $order["type"] == 3) {
				$add_money = \floatval($order["total"]) - \floatval($order["s_money"]) - \floatval($order["fxs_money"]) + $order["food_money"];
			} else {
				$add_money = \floatval($order["total"]) - \floatval($order["s_money"]) - \floatval($order["fxs_money"]);
	...
	public function delivered()
	{
		...
		$res = \think\facade\Db::name("dmh_school_order")->where(["id" => $o_id, "wxapp_id" => $wxapp_id])->update(["status" => 11]);
		if ($res) {
			return $this->ajaxReturn($this->successCode, "操作成功");
		} else {
			return $this->ajaxReturn($this->errorCode, "操作失败");
