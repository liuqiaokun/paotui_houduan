...
class Order extends Common
{
	...
	{
		$postField = "a_id,unique,s_id,wxapp_id,total,img,remarks,sex_limit,weight,express_num,sh_addres,qu_longitude,qu_latitude,co_id,out_time_num,start_time,good_details,food_money,store_id,MinRunPrice,goods_list";
		$data = $this->request->only(explode(",", $postField), "post", null);
		if ($data["unique"]) {
			$cache = \think\facade\Cache::get($data["unique"]);
			if ($cache) {
	...
	public function grab()
	{
		...
		if ($order["is_open_pay"] == 1 && $order["type"] == 3) {
			\app\api\model\Order::where(["id" => $o_id])->update(["end_openid" => $user["openid"], "status" => 3, "s_money" => $plat_rate, "fxs_money" => $school_rate, "store_money" => 0, "fx_store_money" => 0]);
		} else {
			...
			\app\api\model\Order::where(["id" => $o_id])->update(["end_openid" => $user["openid"], "status" => 3, "s_money" => $plat_rate, "fxs_money" => $school_rate, "store_money" => $store_rate, "fx_store_money" => $fx_store_rate]);
		}
		if ($order["type"] == 3) {
			Printer::index($o_id);
	...
	public function StorePrint($o_id, $wxapp_id)
	{
		...
		if ($order["is_open_pay"] == 1) {
			...
			$store_rate = \floatval($order["food_money"]) * \floatval($school["store_rate"]) / 100;
			$fx_store_rate = \floatval($order["food_money"]) * \floatval($school["fx_store_rate"]) / 100;
